...
function JobsMarqueeBand() {
  const segment = `${jobsMarqueePhrases.join(" · ")} · `;
  return (
...
export function JobsExperience() {
  ...
  const activeJob = useMemo(
    () => jobsOpenings.find((job) => job.id === activeJobId) ?? null,
    [activeJobId],
  );
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.fullName.trim() || !form.email.trim() || !form.motivation.trim()) {
      setErrorMsg(jobsUI.errors.required[locale]);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setErrorMsg(jobsUI.errors.submit[locale]);
        return;
      }

      setSuccessMsg(jobsUI.success[locale]);
      setForm(EMPTY_FORM);
    } catch {
      setErrorMsg(jobsUI.errors.submit[locale]);
    } finally {
        ...
        <section className="relative z-10 border-b border-pp-olive/10 px-5 py-7 sm:px-8 md:py-8">
          <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 rounded-none border border-pp-olive/14 bg-[#fbf8ec] p-4 shadow-[0_14px_36px_rgb(28_56_52/0.1)] sm:flex-row sm:items-center sm:justify-between sm:gap-5 sm:p-5">
            <div className="min-w-0">
              <h1 className="text-3xl text-pp-olive sm:text-[2rem]">{jobsPageCopy.title}</h1>
              <p className="mt-2 max-w-3xl text-sm leading-relaxed text-pp-black/74">
                {jobsPageCopy.intro[locale]}
              </p>
            </div>
            <div className="flex shrink-0 flex-wrap items-center gap-2">
              <a
                href={`mailto:${jobsPageCopy.mailto}`}
              ...
              >
                {jobsPageCopy.ctaPrimary[locale]}
              </a>
              <Link
                href="/locations"
              ...
              >
                {jobsPageCopy.ctaSecondary[locale]}
              </Link>
            </div>
          </div>
                ...
                <div className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
                  <div className="space-y-4">
                    <section className="rounded-none border border-pp-olive/12 bg-pp-white/55 p-3">
                      <h4 className="text-[0.62rem] tracking-[0.18em] text-pp-olive uppercase">{jobsUI.whatYouDo[locale]}</h4>
                    ...
                    <section className="rounded-none border border-pp-olive/12 bg-pp-white/55 p-3">
                      <h4 className="text-[0.62rem] tracking-[0.18em] text-pp-olive uppercase">{jobsUI.profile[locale]}</h4>
                      <ul className="mt-2 space-y-1.5">
                    ...
                    <section className="rounded-none border border-pp-olive/12 bg-pp-white/55 p-3">
                      <h4 className="text-[0.62rem] tracking-[0.18em] text-pp-olive uppercase">{jobsUI.offer[locale]}</h4>
                      <ul className="mt-2 space-y-1.5">
                    ...
                    <section className="rounded-none border border-pp-olive/12 bg-pp-white/55 p-3">
                      <h4 className="text-[0.62rem] tracking-[0.18em] text-pp-olive uppercase">{jobsUI.practical[locale]}</h4>
                      <ul className="mt-2 space-y-1.5">
                    ...
                    <section className="rounded-none border border-pp-olive/12 bg-pp-white/55 p-3">
                      <h4 className="text-[0.62rem] tracking-[0.18em] text-pp-olive uppercase">{jobsUI.process[locale]}</h4>
                  ...
                  <section className="rounded-none border border-pp-olive/14 bg-pp-white/68 p-3">
                    <h4 className="text-[0.62rem] tracking-[0.18em] text-pp-lollypop uppercase">{jobsUI.formTitle[locale]}</h4>
                    <p className="mt-1 text-sm text-pp-black/72">{jobsUI.formIntro[locale]}</p>
                    ...
                    <form className="mt-3 space-y-2.5" onSubmit={handleSubmit}>
                      ...
                      <div>
                        <label className="mb-1 block text-[0.62rem] tracking-[0.16em] text-pp-olive/78 uppercase">
                          {jobsUI.labels.preferredLocation[locale]}
                        </label>
                        <input
                          className={fieldClass()}
                          placeholder={jobsUI.placeholders.preferredLocation[locale]}
                      ...
                      <div>
                        <label className="mb-1 block text-[0.62rem] tracking-[0.16em] text-pp-olive/78 uppercase">
                          {jobsUI.labels.availability[locale]}
                        </label>
                        <input
                          className={fieldClass()}
                          placeholder={jobsUI.placeholders.availability[locale]}
                      ...
                      <div>
                        <label className="mb-1 block text-[0.62rem] tracking-[0.16em] text-pp-olive/78 uppercase">
                          {jobsUI.labels.motivation[locale]} *
                        </label>
                        <textarea
                          className={`${fieldClass()} min-h-24 resize-y`}
                          placeholder={jobsUI.placeholders.motivation[locale]}
                      ...
                      <div>
                        <label className="mb-1 block text-[0.62rem] tracking-[0.16em] text-pp-olive/78 uppercase">
                          {jobsUI.labels.cvUrl[locale]}
                        </label>
                        <input
                          className={fieldClass()}
                          placeholder={jobsUI.placeholders.cvUrl[locale]}
                          value={form.cvUrl}
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                        ...
                        >
                          {sending ? jobsUI.submit.sending[locale] : jobsUI.submit.idle[locale]}
                        </button>
                      </div>
                    </form>
                  </section>
